...
"""
    ...
    To Do:
    1 - add flag for too many subsitutions and too few total sites
...
def LIGMOD(core, ligand_dict, mod_dict, param_dict):
    ...
    for envir_num, lig_envir in ligand_dict.items():
        ...
        for i in numsub:
            ...
            #elif i == 1:
            # if i == 1, then flatten all list of lists, take the first value from each, and add to ind_perm
                ...
                for jj in ind_perm:
                    flag = False
                    for jjj in jj:
                        if any(isinstance(el, list) for el in jjj[1]):
                            tempp = list(itertools.chain(*jjj[1]))
                        else:
                            tempp = jjj[1]

                        if jj.count(jjj) > len(tempp):
                            flag = True

                    if flag == False and jj not in temp:
                        temp.append(jj)
                ...
                for jj in temp:
                    ...
                    while jjj > -i:
                        ...
                        #if any(isinstance(el, list) for el in jj[jjj][1]):
                        #    for kk in range(len(jj[jjj][1])):
                        #        print (jj[jjj][0], jj[jjj][1][-kk][-i])
                        #else:
                        #    print (jj[jjj][0], jj[jjj][1])
                        #jjj -= 1
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == 'long':
        ...
        ligand_dict = {0:
                     [{'acac': {'Number of Ligands': '2', 'Symmetric Hs': [[8, 9, 10], [11, 12, 13]], 'Non-Symmetric Hs': [14]}}],
                     }
